...
const mockedAddRepositoryCollaborator =
  vi.mocked(
    addRepositoryCollaborator,
  );

const mockedGetRepositoryCollaborators =
  vi.mocked(
    getRepositoryCollaborators,
  );

const mockedUpdateRepositoryCollaborator =
  vi.mocked(
    updateRepositoryCollaborator,
  );

const mockedRemoveRepositoryCollaborator =
  vi.mocked(
    removeRepositoryCollaborator,
  );
...
describe(
  'repository collaborator API integration',
  () => {
    beforeEach(() => {
      vi.resetAllMocks();
    });
    ...
    it('adds a repository collaborator', async () => {
      mockedAddRepositoryCollaborator
        .mockResolvedValue(
          collaborator,
        );
      ...
      const response =
        await request(app)
          .post(
            '/api/repositories/asil/demo/collaborators',
          )
          .send({
            username: 'testuser',
            permission: 'READ',
    ...
    it('lists repository collaborators', async () => {
      mockedGetRepositoryCollaborators
        .mockResolvedValue([
          collaborator,
          {
            ...collaborator,
    ...
    it('updates repository collaborator permission', async () => {
      ...
      mockedUpdateRepositoryCollaborator
        .mockResolvedValue(
          updatedCollaborator,
        );
    ...
    it('removes a repository collaborator', async () => {
      mockedRemoveRepositoryCollaborator
        .mockResolvedValue(
          undefined,
        );
      ...
      expect(
        ...
        success: true,
        message:
          'Repository collaborator removed successfully',
      });
